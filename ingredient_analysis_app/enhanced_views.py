            ...
            try:
                ...
                if rag_service_instance:
                    try:
                        ...
                        print("DEBUG retrieved docs:", [doc.metadata for doc in relevant_docs])
...
def rag_status(request):
    ...
    status_info = {
        ...
        "vectorstore_loaded": (rag_service_instance.documents is not None and len(rag_service_instance.documents) > 0) if rag_service_instance else False,
...
def rag_dashboard(request):
    ...
    doc_count = 0
    docs_list = []
    if rag_service_instance and rag_service_instance.documents:
        doc_count = len(rag_service_instance.documents)
        docs_list = [doc.metadata.get("source", "Unknown") for doc in rag_service_instance.documents[:10]]
    ...
    return render(request, "rag_dashboard.html", context)
